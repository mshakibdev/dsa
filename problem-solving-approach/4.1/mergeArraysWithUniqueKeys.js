...
function mergeArraysByKey(arr1, arr2, key1, key2) {
  // declaring two arrays
  const arrayObj = [];
  const arrayObj2 = [];

  // loop through input arr1
  arr1.forEach((profileObject) => {
    profileObject.hasOwnProperty(`${key1}` || `${key2}`)
      ? arrayObj.push(profileObject)
      : {};
  });
  ...
  arr2.forEach((profileObject) => {
    ...
    arrayObj.filter((obj) => {
      if (
        obj[key1] === profileObject[key1] ||
        obj[key2] === profileObject[key2]
      ) {
